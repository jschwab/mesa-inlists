...
! ***********************************************************************
      ...
      subroutine summerschool_energy_routine(id, ierr)
         ...
         integer :: k
         real(dp) :: trel, mtransition, etatransition
         real(dp) :: highentropy, lowentropy, tempWD
         ...
         do k = 1, s% nz
            ...
            if (s% m(k) > mtransition) then
               s% extra_heat(k) = ( highentropy - exp_cr(s%lnS(k)) )*s%T(k)/trel
            end if
            ...
            if (s% m(k) .le. mtransition .and. s%eta(k) < etatransition) then
               s% extra_heat(k) = ( lowentropy - exp_cr(s%lnS(k)) )*s%T(k)/trel
            end if
            ...
            if (s% m(k) .le. mtransition .and. s%eta(k) .ge. etatransition) then
               s% extra_heat(k) = ( tempWD - s%T(k) )*exp_cr(s%lnS(k))/trel
            end if
         end do
